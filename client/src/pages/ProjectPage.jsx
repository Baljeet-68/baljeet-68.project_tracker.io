...
export default function ProjectPage() {
  ...
  const [sectionsOpen, setSectionsOpen] = useState({ summary: false, description: false, timeline: false, team: true, milestones: false, activity: false })
  const user = getUser()
  ...
  // overall progress percentage for header
  const progressPercentage = useMemo(() => {
    if (!screensList) return 0;
    const total = screensList.length;
    const done = screensList.filter(s => s.status === 'Done').length;
  ...
  return (
    <PageContainer>
      <div className="container-fluid py-4">
        ...
        <div className="w-full max-w-full px-3 mb-8">
          {/* New header layout */}
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-3 mt-2">
                {project?.status && (
                  <Badge gradient={getStatusGradient(project?.status)} size="sm">
                    {project?.status}
                  </Badge>
                )}
              ...
              <div className="mt-2">
                <ProgressBar value={progressPercentage} className="h-2 rounded-full" />
              </div>
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-xs text-slate-600">
                <div>
                  Tester: <span className="font-semibold">{project?.testerName || '—'}</span>
                </div>
                <div>
                  Developers: <span className="font-semibold">{project?.developerNames?.length || 0}</span>
                </div>
                <div>
                  Bugs: <span className="font-semibold">{project?.openBugsCount || 0}</span>
                </div>
                <div>
                  Screens: <span className="font-semibold">{screensList.length}</span>
                </div>
              </div>
            </div>
            <div className="flex-none">
              <ActionDropdown
                label="Actions"
                options={[
                  { label: 'View Overview', onClick: () => setTabIndex(0) },
                  { label: 'View Screens', onClick: () => setTabIndex(1) },
                  { label: 'View Bugs', onClick: () => setTabIndex(2) },
                  { label: 'View Tasks', onClick: () => setTabIndex(3) },
                  { label: 'Refresh Data', onClick: loadData }
                ]}
              />
            </div>
          </div>
              ...
              <div className="flex flex-wrap -mx-3">
                <div className="flex items-center w-full max-w-full px-3 shrink-0 md:w-8/12 md:flex-none">
                  <ul className="flex flex-wrap p-1 list-none bg-gray-50 rounded-xl" role="tablist">
                    {['Overview', 'Screens', 'Bugs', 'Tasks', 'Documents'].map((tab, idx) => (
            ...
            <CardBody className="p-6">
              ...
              {tabIndex === 0 && (
                <div className="space-y-8">
                    ...
                    <div className="relative w-24 h-24">
                      <svg className="w-full h-full" viewBox="0 0 36 36">
                        ...
                        <path
                          ...
                          d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                        />
                        <text x="18" y="20.35" className="text-xs font-bold text-center fill-current text-green-600" textAnchor="middle">
                          {progressPercentage}%
                        </text>
                      </svg>
                    </div>
                  ...
                  {/* Summary section */}
                  <div>
                    <button onClick={() => toggleSection('summary')} className="flex items-center gap-2 font-bold">
                      <ChevronRight className={`${sectionsOpen.summary ? 'rotate-90' : ''}`} />
                      Project Summary
                    </button>
                    {sectionsOpen.summary && (
                      <div className="p-4 bg-white rounded-lg shadow-soft">{generateProjectSummary()}</div>
                    )}
                  </div>

                  {/* Description section */}
                  <div>
                    <button onClick={() => toggleSection('description')} className="flex items-center gap-2 font-bold">
                      <ChevronRight className={`${sectionsOpen.description ? 'rotate-90' : ''}`} />
                      Description
                    </button>
                    {sectionsOpen.description && (
                      <div className="p-4 bg-white rounded-lg shadow-soft">{project?.description || 'No description provided.'}</div>
                    )}
                  </div>

                  {/* Timeline section */}
                  <div>
                    <button onClick={() => toggleSection('timeline')} className="flex items-center gap-2 font-bold">
                      <ChevronRight className={`${sectionsOpen.timeline ? 'rotate-90' : ''}`} />
                      Timeline
                    </button>
                    {sectionsOpen.timeline && (
                      <div className="p-4 bg-white rounded-lg shadow-soft grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><strong>Start:</strong> {formatDateDisplay(project?.startDate)}</div>
                        <div><strong>End:</strong> {formatDateDisplay(project?.endDate)}</div>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Team Members */}
                  <div>
                    <button onClick={() => toggleSection('team')} className="flex items-center gap-2 font-bold">
                      <ChevronRight className={`${sectionsOpen.team ? 'rotate-90' : ''}`} />
                      Team Members
                    </button>
                    {sectionsOpen.team && (
                      <div className="p-4 bg-white rounded-lg shadow-soft">
                        {(project?.developerNames || []).map(dev => (
                          <div key={dev.id} className="py-1">{dev.name}</div>
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  {/* Milestones */}
                  <div>
                    <button onClick={() => toggleSection('milestones')} className="flex items-center gap-2 font-bold">
                      <ChevronRight className={`${sectionsOpen.milestones ? 'rotate-90' : ''}`} />
                      Milestones
                    </button>
                    {sectionsOpen.milestones && (
                      <div className="p-4 bg-white rounded-lg shadow-soft">
                        {(milestonesList || []).map(m => (
                          <div key={m.id} className="py-1">{m.milestoneNumber}: {m.status}</div>
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  {/* Activity Feed */}
                  <div>
                    <button onClick={() => toggleSection('activity')} className="flex items-center gap-2 font-bold">
                      <ChevronRight className={`${sectionsOpen.activity ? 'rotate-90' : ''}`} />
                      Activity Feed
                    </button>
                      ...
                      <div className="p-4 bg-white rounded-lg shadow-soft">
                        {activityList.length === 0 ? (
                          ...
                          activityList.map((act, idx) => (
                            <div key={idx} className="py-1">{act.description || 'Update'}</div>
                          ))
                        )}
                      </div>
                    )}
